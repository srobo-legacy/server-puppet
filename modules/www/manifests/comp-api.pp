...
class www::comp-api ( $git_root, $root_dir ) {
  ...
  # SimpleJSON so we can handle Decimal()s, PIP so we can get Flask from it.
  package { ['python-simplejson', 'python-pip']:
    ensure => present,
    notify => Service['httpd'],
    before => Vcsrepo[$root_dir],
  }

  # need better than version 0.8, which is all that's available in the repos
  package { ['Flask']:
    ensure => present,
    provider => pip,
